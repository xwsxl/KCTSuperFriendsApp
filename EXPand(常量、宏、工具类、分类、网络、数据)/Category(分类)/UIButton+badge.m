...
//  aTaohMall
//
//  Created by Zhending Shi on 2017/9/19.
...
{
    // 初始化，设定默认值
    self.badgeBGColor   = APPOINTCOLORFifth;
    self.badgeTextColor = [UIColor whiteColor];
    self.badgeFont      = [UIFont systemFontOfSize:12.0];
    self.badgePadding   = 6;
    self.badgeMinSize   = 8;
    
    self.badgeOriginX=(self.frame.size.width)/2+CWidth(5);
    self.badgeOriginY   = (self.frame.size.height-25)/2-20;
...
-(void) setBadgeValue:(NSString *)badgeValue
{
    ...
    // 当角标信息不存在，或者为空，则移除
    if (!badgeValue || [badgeValue containsString:@"null"] || [badgeValue isEqualToString:@""] || ([badgeValue isEqualToString:@"0"])) {
        [self removeBadge];
    } else if (!self.badge) {
        //当又有值时，重新设置角标
