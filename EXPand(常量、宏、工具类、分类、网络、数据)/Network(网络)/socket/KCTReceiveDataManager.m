...
-(void)dealWithMessage:(id)message
{
        ...
        XLLog(@"解析后的数据:%@",dic);
        if ([dic[@"type"] integerValue]!=20&&![KCUserDefaultManager getIsTimServer]) {
...
-(void)dealWithSingleMessage:(NSDictionary *)dic
{
    ...
    NSDictionary *data;
    XLLog(@"type=%@,%@",[dic[@"text"] class],[NSDictionary class]);
    if ([[dic[@"text"] className] isEqualToString:@"__NSDictionaryM"]) {
        data=dic[@"text"];
    }else
    {
        NSString *datastring=dic[@"text"];
        NSData *data2 = [[NSData alloc] initWithData:[datastring dataUsingEncoding:NSUTF8StringEncoding]];
        data=[NSJSONSerialization JSONObjectWithData:data2 options:NSJSONReadingMutableContainers error:nil];
    }
    ...
    model.msgType=[data[@"type"] integerValue];
    ...
    model.msg=d[@"url"];
    model.duration=d[@"duration"]?([d[@"duration"] floatValue]+999)/1000:0;
    ...
    {
        RoomRLMModel *room=rs2.lastObject;
        rModel.unreadNum=room.unreadNum+1;
    }
    rModel.chat=model;
...
/* ****  处理群聊聊天消息  **** */
-(void)dealWithGroupMessage:(NSDictionary *)dic
{
    NSInteger count=[[UIApplication sharedApplication] applicationIconBadgeNumber];
    [[UIApplication sharedApplication] setApplicationIconBadgeNumber:count+1];
    MessageRLMModel *model=[[MessageRLMModel alloc] init];
    ...
    ContactRLMModel *Cmodel=[[ContactRLMModel alloc] init];
    RLMResults *rs2=[ContactRLMModel objectsWhere:[NSString stringWithFormat:@"account contains '%@'",data[@"from_account"]]];
    if (rs2.count>0) {
        Cmodel=rs2.lastObject;
        model.contact=Cmodel;
    }else
    {
    ...
    GroupRLMModel *Gmodel=[[GroupRLMModel alloc] init];
    RLMResults *rs=[GroupRLMModel objectsWhere:[NSString stringWithFormat:@"groupNum contains '%@'",data[@"groupNum"]]];
    model.roomNo=[[NSString stringWithFormat:@"%@_%@",[KCUserDefaultManager getAccount],data[@"groupNum"]] md5];
    if (rs.count>0) {
        Gmodel=rs.lastObject;
    }else
    {
        Gmodel.groupName=data[@"groupName"];
        Gmodel.groupType=data[@"groupType"];
        Gmodel.groupNum=data[@"groupNum"];
        Gmodel.groupPhoto=data[@"groupPhoto"];
    }
  
    
    /* ****  获取本地是否有房间号、没有则创建房间  **** */
    RLMResults *rs3=[RoomRLMModel objectsWhere:[NSString stringWithFormat:@"roomNo contains '%@'",model.roomNo]];
    RoomRLMModel *rModel;
    rModel=[[RoomRLMModel alloc] init];
    rModel.roomNo=model.roomNo;
    rModel.timestamp=model.timeStamp;
    ...
    {
        RoomRLMModel *room=rs3.lastObject;
        rModel.unreadNum=room.unreadNum+1;
    }
    rModel.chat=model;
    [KCTRealmManager addOrUpdateObject:rModel];
