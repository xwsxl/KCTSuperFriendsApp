...
@interface KCWebSocketManager()<SRWebSocketDelegate>
{
    int _index;
    NSTimer * heartBeat;
...
@implementation KCWebSocketManager

+(KCWebSocketManager *)instance{
    static KCWebSocketManager *Instance = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        Instance = [[KCWebSocketManager alloc] init];
    });
    return Instance;
}
...
-(void)SRWebSocketOpen{
    ...
    NSString *url=KNSSTR(@"/sysController/getClientAddress");
    
    [KCNetWorkManager POST:url WithParams:@{} ForSuccess:^(NSDictionary * _Nonnull response) {
        if ([response[@"code"] integerValue]==200) {
         // /*
            NSString *AUTH=[NSString stringWithFormat:@"%@%@",[KCUserDefaultManager getSigen],[KCUserDefaultManager getAccount]];
            urlstr=[NSString stringWithFormat:@"%@/?account=%@&auth=%@&type=2",response[@"data"],[KCUserDefaultManager getAccount],[AUTH md5]];
            self.socket = [[SRWebSocket alloc] initWithURLRequest:
                           [NSURLRequest requestWithURL:[NSURL URLWithString:urlstr]]];//这里填写你服务器的地址
            XLLog(@"请求的websocket地址：%@",self.socket.url.absoluteString);
...
- (void)sendData:(id)data {
    ...
    dispatch_async(queue, ^{
        if (weakSelf.socket != nil) {
            ...
            XLLog(@"没网络，发送失败，一旦断网 socket 会被我设置 nil 的");
            XLLog(@"其实最好是发送前判断一下网络状态比较好，我写的有点晦涩，socket==nil来表示断网");
        }
    });
}
...
- (void)webSocket:(SRWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(NSString *)reason wasClean:(BOOL)wasClean {
    ...
    if (webSocket == self.socket) {
        XLLog(@"************************** socket连接断开************************** ");
        XLLog(@"被关闭连接，code:%ld,reason:%@,wasClean:%d",(long)code,reason,wasClean);
        [self SRWebSocketClose];
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message  {
    ...
    if (webSocket == self.socket) {
        XLLog(@"************************** socket收到数据了************************** ");
        XLLog(@"解析前的数据:%@",message);
       [[KCTReceiveDataManager instance] dealWithMessage:message];
    }
}
