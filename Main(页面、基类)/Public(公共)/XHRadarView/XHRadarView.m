...
#define INDICATOR_START_COLOR [UIColor colorWithRed:28.0/255.0 green:89.0/255.0 blue:132.0/255.0 alpha:0.5]
#define INDICATOR_END_COLOR [UIColor colorWithRed:28.0/255.0 green:89.0/255.0 blue:132.0/255.0 alpha:0]
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i=0; i<sectionsNum ; i++) {
        ...
        CGContextAddArc(context, self.center.x, self.center.x+KSafeAreaTopNaviHeight, sectionRadius - 5*(sectionsNum - i - 1), 0, 2*M_PI, 0); //添加一个圆
...
//
//        // 逆时针画一个圆弧
//        CGContextAddArc(context, self.center.x, self.center.y, sectionRadius - 5*(sectionsNum - i - 1), 0, M_PI * 2, 1);
//        // 2. 创建一个渐变色
...
//
//        // 4. 用渐变色填充
//        CGContextDrawLinearGradient(context, gradient, CGPointMake(self.center.x, 0), CGPointMake(self.center.x, self.center.y+sectionRadius-5*(sectionsNum - i - 1)), 0);
//        // 释放渐变色
...
//        CGContextDrawPath(context, kCGPathStroke); //绘制路径
    ...
    if (self.indicatorView) {
        self.indicatorView.frame=CGRectMake(0, 0,self.radius*2, self.radius*2);
        self.indicatorView.center=CGPointMake(self.center.x, self.center.x+KSafeAreaTopNaviHeight);
        ...
        for (int index=0; index<pointsNum; index++) {
            if (self.dataSource && [self.dataSource respondsToSelector:@selector(radarView:viewForIndex:)]) {
                if (self.dataSource && [self.dataSource respondsToSelector:@selector(radarView:positionForIndex:)]) {
                    ...
                    XLLog(@"sin:%f, cos:%f, self center:%@, point center:%@",sin(DEGREES_TO_RADIANS(posDirection)), cos(DEGREES_TO_RADIANS(posDirection)), NSStringFromCGPoint(self.center), NSStringFromCGPoint(pointView.center));
...
- (void)didSelectItemRadarPointView:(XHRadarPointView *)radarPointView {
    XLLog(@"select point %d", radarPointView.tag);
