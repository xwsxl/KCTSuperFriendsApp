...
-(instancetype)initWithItem:(SSImageGroupItem *)item{
    if(self = [super init]){
        ...
        XLLog(@"本地视频地址%@",_item.videoPath);
        NSFileManager *fileManager=[NSFileManager defaultManager];
        NSString *path=[[KCFileManager getUserChatVideoDirectory] stringByAppendingPathComponent:_item.videoPath];
        ...
        {
            [[KCTDownLoadManager sharedInstance] downLoadVideoWithRequestUrl:[KNSPHOTOURL(_item.videoPath) absoluteString] complete:^(BOOL success, id obj) {
                AVPlayerItem *playerItem = [AVPlayerItem playerItemWithURL:[NSURL fileURLWithPath:path]];
                _player = [AVPlayer playerWithPlayerItem:playerItem];
