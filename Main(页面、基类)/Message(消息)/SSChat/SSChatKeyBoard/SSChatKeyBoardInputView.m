...
- (void)beginRecordVoice:(UIButton *)button{
    ...
    NSDictionary *recordSetting = @{AVEncoderAudioQualityKey : [NSNumber numberWithInt:AVAudioQualityMin],
                                    AVSampleRateKey : [NSNumber numberWithInt:16000],
...
//    _docmentFilePath = [NSString stringWithFormat:@"%@/%@.aac",docments,@"123"];
    _docmentFilePath = [NSString stringWithFormat:@"%@/%ld.aac",[KCFileManager getUserChatAudioDirectory],[NSString getTimeStamp]];
    
    NSURL *pathURL = [NSURL fileURLWithPath:self.docmentFilePath];
    _recorder = [[AVAudioRecorder alloc] initWithURL:pathURL settings:recordSetting error:&error];
    if (error || !_recorder) {
        XLLog(@"recorder: %@ %zd %@", [error domain], [error code], [[error userInfo] description]);
        return;
    }
...
- (void)endConvertWithData:(NSData *)voiceData
{
    if(_delegate && [_delegate respondsToSelector:@selector(SSChatKeyBoardInputViewBtnClick:sendVoice:time:)]){
        [self.delegate SSChatKeyBoardInputViewBtnClick:self sendVoice:_docmentFilePath time:_playTime+1];
    }
