...
+ (SSChatIMEmotionModel *)modelWithDictionary:(NSDictionary*)dic atIndex:(int)index{
    ...
    model.imageName = [NSString stringWithFormat:@"Expression_%d@2x", index+1];
...
- (void)initEmotionImages{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        UIImage *img =[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:model.imageName ofType:@"png"]];
...
-(NSMutableAttributedString *)emotionImgsWithString:(NSString *)string{
    ...
    if (!re) {
        XLLog(@"%@", [error localizedDescription]);
    }
