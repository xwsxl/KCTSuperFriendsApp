...
- (void)show
{
	...
	self.titleLabel.center = CGPointMake(SCREEN_Width/2,SCREEN_Height/2 - 30);
	self.titleLabel.text = @"倒计时";
	
	self.subTitleLabel.center = CGPointMake(SCREEN_Width/2,SCREEN_Height/2 + 30);
	self.subTitleLabel.text = @"向上滑动取消";
	
	self.centerLabel.center = CGPointMake(SCREEN_Width/2,SCREEN_Height/2);
	self.centerLabel.text = @"120";
...
{
	if (!_edgeImageView) {
		_edgeImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Chat_record_circle"]];
